...
export function generateTemplate20Html({ formData }: { formData: JobPostFormData }): string {
  if (!formData) return '<h1>Loading...</h1>';

  // Modern vibrant color scheme
  const primaryColor = '#7B2CBF';
  const secondaryColor = '#240046';
  const accentColor = '#FF9E00';
  const darkColor = '#10002B';
  const lightColor = '#FAF5FF';
