...
export function generateTemplate37Html({ formData }: { formData: JobPostFormData }): string {
  if (!formData) return '<h1>Loading...</h1>';

  // Vibrant Coral-Teal Color Scheme
  const primaryColor = '#F97316'; // Orange & Indigo Primary
  const secondaryColor = '#6366F1'; // Orange & Indigo Secondary
  const accentColor = '#FDE047';
  const darkColor = '#431407';
  const lightColor = '#FFF7ED';
  const pinkColor = '#A5B4FC';
