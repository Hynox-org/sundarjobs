...
export function generateTemplate29Html({ formData }: { formData: JobPostFormData }): string {
  if (!formData) return '<h1>Loading...</h1>';

  // Bold Green-Yellow Color Scheme
  const primaryColor = '#78350F'; // Brown & Cyan Primary
  const secondaryColor = '#06B6D4'; // Brown & Cyan Secondary
  const accentColor = '#92400E';
  const darkColor = '#451A03';
  const lightColor = '#ECFEFF';
  const textDark = '#451A03';
