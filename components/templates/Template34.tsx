...
export function generateTemplate34Html({ formData }: { formData: JobPostFormData }): string {
  if (!formData) return '<h1>Loading...</h1>';

  // Vibrant Coral-Teal Color Scheme
  const primaryColor = '#10B981'; // Emerald & Ruby Primary
  const secondaryColor = '#F43F5E'; // Emerald & Ruby Secondary
  const accentColor = '#F59E0B';
  const darkColor = '#064E3B';
  const lightColor = '#ECFDF5';
