...
export function generateTemplate32Html({ formData }: { formData: JobPostFormData }): string {
  if (!formData) return '<h1>Loading...</h1>';

  // Vibrant Coral-Teal Color Scheme
  const primaryColor = '#0EA5E9'; // Oceanic Blue & Lime Primary
  const secondaryColor = '#84CC16'; // Oceanic Blue & Lime Secondary
  const accentColor = '#FDE047';
  const darkColor = '#0C4A6E';
  const lightColor = '#F0F9FF';
  const pinkColor = '#A7F3D0';
