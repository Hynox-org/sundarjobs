...
export function generateTemplate38Html({ formData }: { formData: JobPostFormData }): string {
  if (!formData) return '<h1>Loading...</h1>';

  // Vibrant Coral-Teal Color Scheme
  const primaryColor = '#EF4444'; // Red & Cyan Primary
  const secondaryColor = '#06B6D4'; // Red & Cyan Secondary
  const accentColor = '#FEF08A';
  const darkColor = '#7F1D1D';
  const lightColor = '#FEF2F2';
  const pinkColor = '#67E8F9';
