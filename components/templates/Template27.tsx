...
export function generateTemplate27Html({ formData }: { formData: JobPostFormData }): string {
  if (!formData) return '<h1>Loading...</h1>';

  // Bold Green-Yellow Color Scheme
  const primaryColor = '#9F1239'; // Maroon & Gold Primary
  const secondaryColor = '#D97706'; // Maroon & Gold Secondary
  const accentColor = '#F43F5E';
  const darkColor = '#4C0519';
  const lightColor = '#FFF1F2';
  const textDark = '#4C0519';
