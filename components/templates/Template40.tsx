...
export function generateTemplate40Html({ formData }: { formData: JobPostFormData }): string {
  if (!formData) return '<h1>Loading...</h1>';

  // Vibrant Coral-Teal Color Scheme
  const primaryColor = '#64748B'; // Slate & Bronze Primary
  const secondaryColor = '#D97706'; // Slate & Bronze Secondary
  const accentColor = '#CBD5E1';
  const darkColor = '#0F172A';
  const lightColor = '#F8FAFC';
  const pinkColor = '#FCD34D';
