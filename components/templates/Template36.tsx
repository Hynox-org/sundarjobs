...
export function generateTemplate36Html({ formData }: { formData: JobPostFormData }): string {
  if (!formData) return '<h1>Loading...</h1>';

  // Vibrant Coral-Teal Color Scheme
  const primaryColor = '#14B8A6'; // Teal & Magenta Primary
  const secondaryColor = '#D946EF'; // Teal & Magenta Secondary
  const accentColor = '#FCD34D';
  const darkColor = '#134E4A';
  const lightColor = '#F0FDFA';
  const pinkColor = '#F0ABFC';
