...
export function generateTemplate35Html({ formData }: { formData: JobPostFormData }): string {
  if (!formData) return '<h1>Loading...</h1>';

  // Vibrant Coral-Teal Color Scheme
  const primaryColor = '#3B82F6'; // Midnight & Gold Primary
  const secondaryColor = '#EAB308'; // Midnight & Gold Secondary
  const accentColor = '#F59E0B';
  const darkColor = '#1E3A8A';
  const lightColor = '#EFF6FF';
  const pinkColor = '#93C5FD';
