...
export function generateTemplate11Html({ formData }: { formData: JobPostFormData }): string {
  if (!formData) return '<h1>Loading...</h1>';

  // Modern vibrant color scheme
  const primaryColor = '#606C38';
  const secondaryColor = '#BC4749';
  const accentColor = '#283618';
  const darkColor = '#1A1A1A';
  const lightColor = '#FEFAE0';
